...
import cn.com.flaginfo.module.reflect.annotation.CvtField;
import cn.com.flaginfo.module.reflect.annotation.CvtTransformHandler;
...
public class BeansConverterUtils {
    ...
    public static <S, T> List<T> converter(List<S> sources, Class<T> tClass, boolean withCvt, boolean order) {
        ...
        return sources.stream().filter(Objects::nonNull).map(s -> converterNotNullObject(s, tClass, withCvt)).collect(Collectors.toList());
    }
    ...
    public static <S, T> void converterCustomField(S source, T tClass) {
        ...
        classDescription.getFieldsMap().values().stream()
